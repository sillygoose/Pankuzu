...
actor AccuracyLocationFilter: LocationFilter {
  ...
  func shouldAccept(_ location: CLLocation) -> Bool {
    guard location.horizontalAccuracy >= 0 && location.verticalAccuracy >= 0 else {
      DokoLogging.shared.postLoggingResponse(.coreLocation("AccuracyLocationFilter.invalid"))
      return false
    }
    guard location.horizontalAccuracy < horizontalAccuracyThreshold else {
      DokoLogging.shared.postLoggingResponse(.coreLocation("AccuracyLocationFilter.horizontalAccuracy(\(String(format: "%.0f", location.horizontalAccuracy))"))
      return false
    }
    guard location.verticalAccuracy < verticalAccuracyThreshold else {
      DokoLogging.shared.postLoggingResponse(.coreLocation("AccuracyLocationFilter.verticalAccuracy(\(String(format: "%.0f", location.verticalAccuracy))"))
      return false
    }
    return true
  }
